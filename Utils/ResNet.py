...
class ResNet_end2end(nn.Module):
    def __init__(self, in_ch=1, out_channel=2):
        super(ResNet_end2end, self).__init__()
    ...
    def forward(self, x):
        ...
        out12 = out10 + out12
        out12 = self.maxpool12(out12)

        out12 = out12.view(out12.size(0), -1)
        out13 = self.relubn1(self.fcbn1(self.fc1(out12)))
        out13 = self.fc2(out13)
        out_label = F.softmax(out13, dim=1)
...
class ResNet_patch(nn.Module):
    def __init__(self, in_ch=1, out_channel=2):
        super(ResNet_patch, self).__init__()
